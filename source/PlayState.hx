...
class PlayState extends MusicBeatState
{
		...
		switch (curStage)
		{
			...
			case 'philly': //Week 3
				if(!ClientPrefs.lowQuality) {
					...
					var clouds:BGSprite = new BGSprite('philly/tankClouds', FlxG.random.int(-700, -100), FlxG.random.int(-20, 20), 0.1, 0.1);
					clouds.active = true;
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['death_timeline', 'silent_hill', 'fly_away', 'cry_hill',
				'freedom_center', 'sky_base', 'hydrocity', 'majin_forest',
				'sky_sanctuary']);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'death_timeline':
						if(Paths.formatToSongPath(SONG.song) == 'death-timeline' && !usedPractice) {
							unlock = true;
						}
					case 'silent_hill':
						if(Paths.formatToSongPath(SONG.song) == 'silent-hill' && !usedPractice) {
							unlock = true;
						}
					case 'fly_away':
						if(Paths.formatToSongPath(SONG.song) == 'fly-away' && !usedPractice) {
							unlock = true;
						}
					case 'cry_hill':
						if(Paths.formatToSongPath(SONG.song) == 'cry-hill' && !usedPractice) {
							unlock = true;
						}
					case 'freedom center':
						if(Paths.formatToSongPath(SONG.song) == 'freedom-center' && !usedPractice) {
							unlock = true;
						}
					case 'sky_base':
						if(Paths.formatToSongPath(SONG.song) == 'sky-base' && !usedPractice) {
							unlock = true;
						}
					case 'hydrocity':
						if(Paths.formatToSongPath(SONG.song) == 'hydrocity' && !usedPractice) {
							unlock = true;
						}
					case 'majin_forest':
						if(Paths.formatToSongPath(SONG.song) == 'majin-forest' && !usedPractice) {
							unlock = true;
						}
					case 'sky_sanctuary':
						if(Paths.formatToSongPath(SONG.song) == 'sky-sanctuary' && !usedPractice) {
							unlock = true;
						}
				}
